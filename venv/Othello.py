...
#Changes piece based on player turn and coordinates provided
def changePiece(GameBoard, row, col):
...
#Returns a message if selected tile is chosen
def claimedTile():
...
def GamePVP(GameBoard, i, j):
    if playerTurn == Player1.name:
        changeList = isLegal(GameBoard, i, j)
    ...
    if len(changeList) == 0:
        ...
        for i in range(len(changeList)):
            coord = changeList[i]
            changePiece(GameBoard, coord[0], coord[1])
        changeTurn()
        ...
        for j in range(8):
            if GameBoard.board[i][j] == "Blank":
                button = Button(root, height=70, image=pixel, width=69, bg="#c9b493", text="Blank",
                                command=lambda i=i, j=j: GamePVP(GameBoard, i, j, Player1.name,
                                                                 Player2.name))
...
menubar = Menu(root)
menubar.add_command(label="Play Human vs. Human", command=lambda: [welcome.pack_forget(), GameBoardSetup()])
